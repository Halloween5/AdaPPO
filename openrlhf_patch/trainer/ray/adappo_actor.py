...
def _tensor_to_accuracy(value):
    tensor = _to_float_tensor(value)
    if tensor is None:
        return None
    if tensor.dim() > 1:
        tensor = tensor.reshape(tensor.shape[0], -1).mean(dim=1)
    tensor = tensor[torch.isfinite(tensor)]
    if tensor.numel() == 0:
        return None
    if tensor.min().item() >= 0 and tensor.max().item() <= 1:
        return tensor.mean().clamp(0, 1)
    return (tensor > 0).float().mean()


def _extract_rollout_accuracy(experience: "Experience") -> torch.Tensor:
    info = getattr(experience, "info", {}) or {}
    for key in ("accuracy", "accuracies", "acc", "correct", "is_correct"):
        accuracy = _tensor_to_accuracy(info.get(key))
        if accuracy is not None:
            return accuracy.to(experience.action_mask.device)
    for attr in ("scores", "rewards"):
        accuracy = _tensor_to_accuracy(getattr(experience, attr, None))
        if accuracy is not None:
            return accuracy.to(experience.action_mask.device)
    return torch.tensor(0.0, device=experience.action_mask.device)


def _select_adaptive_kl(status: dict) -> float:
    for key in ("kl", "ppo_kl", "vllm_kl"):
        if key in status:
            return _safe_float(status[key])
    return 0.0
...
class ActorPPOTrainer(ABC):
    ...
    def training_step(self, experience: Experience, kl_ctl: float, step: int) -> Dict[str, float]:
        ...
        need_entropy = self.args.actor.entropy_coef is not None or getattr(self.args, "adaptive_critic", False)
        ...
        if need_entropy:
            ...
            # Per-step critic adaptive decision: store entropy in experience.info
            # so critic can read it without an extra forward pass.
            experience.info["entropy"] = entropy_loss.detach()
            if self.args.actor.entropy_coef is not None and self.args.actor.entropy_coef != 0:
        ...
        if "accuracy" not in metrics:
            metrics["accuracy"] = _extract_rollout_accuracy(experience)
            weights["accuracy"] = "sample"
...
class PolicyModelActor(BaseModelActor):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain, max_steps=None, vllm_engines=None):
        ...
        if os.environ.get("PPO_PREWARM_ADAM", "1") == "1":
            _prewarm_adam_states(self.actor_optim)

        if os.environ.get("PPO_DS_ACT_OFFLOAD", "0") == "1":
            _replace_checkpoint_func(self.actor.model)
    ...
    def fit(self, kl_ctl: float = 0):
        ...
        if kl_ctl <= 0:
            kl_ctl = getattr(self.strategy.args.algo.kl, "init_coef", 0.1)
            print(f"[ACTOR] ⚠  kl_ctl=0 — using init_coef={kl_ctl} as fallback")

        kl_target = getattr(self.strategy.args.algo.kl, "target", None)
        if kl_target is not None and kl_target > 0:
            prev_kl = getattr(self, "_last_kl", 0.0)
            if prev_kl > kl_target * 2:
                kl_ctl = min(kl_ctl * 2.0, 10.0)
            elif prev_kl > kl_target:
                kl_ctl = min(kl_ctl * 1.5, 10.0)
            elif prev_kl < kl_target * 0.5 and kl_ctl > getattr(self.strategy.args.algo.kl, "init_coef", 0.1):
                kl_ctl = max(kl_ctl * 0.8, getattr(self.strategy.args.algo.kl, "init_coef", 0.1))

        torch.cuda.empty_cache()
        self.actor.train()
        if os.environ.get("PPO_ACT_OFFLOAD", "0") == "1":
            with _activation_offload_ctx():
                status, step_statuses = self.trainer.ppo_train(kl_ctl)
        else:
            status, step_statuses = self.trainer.ppo_train(kl_ctl)
        ...
        self.actor_update_count = getattr(self, "actor_update_count", 0) + 1
        status["total_actor_updates"] = self.actor_update_count

        if getattr(self.strategy.args, "adaptive_critic", False):
            adaptive_kl = abs(_select_adaptive_kl(status))  # clamp: KL must be non-negative
            self._last_kl = adaptive_kl
            adaptive_accuracy = _safe_float(status.get("accuracy", 0.0))
            adaptive_entropy = _safe_float(status.get("entropy", status.get("entropy_loss", 0.0)))

            # ═══════════════════════════════════════════════════════════════
            kl_danger = getattr(self.strategy.args, "adaptive_kl_danger", 0.5)
            kl_critical = getattr(self.strategy.args, "adaptive_kl_critical", 2.0)
            if adaptive_kl > kl_critical:
                print(f"[ACTOR] ╔══════════════════════════════════════════════╗")
                print(f"[ACTOR] ║  CRITICAL: KL = {adaptive_kl:.4f} > {kl_critical}      ║")
                print(f"[ACTOR] ║  Policy has collapsed. Stop and restart.   ║")
                print(f"[ACTOR] ╚══════════════════════════════════════════════╝")
            elif adaptive_kl > kl_danger:
                print(f"[ACTOR] ⚠  WARNING: KL = {adaptive_kl:.4f} > {kl_danger} — policy diverging")

            truncated_rate = _safe_float(status.get("truncated", 0.0))
            if truncated_rate > 0.9:
                print(f"[ACTOR] ⚠  WARNING: truncated rate = {truncated_rate:.1%} — reward landscape is flat, "
                      f"model cannot learn to produce EOS. Consider reducing --rollout.max_new_tokens.")

            entropy_ema = getattr(self, "adaptive_entropy_ema", adaptive_entropy)
            entropy_collapse = _entropy_collapse(adaptive_entropy, entropy_ema)
            if adaptive_entropy > 0:
                self.adaptive_entropy_ema = 0.9 * entropy_ema + 0.1 * adaptive_entropy

            # ── Accuracy Growth Rate ──
            # Track accuracy over a sliding window. Fast improvement → actor is
            # learning well from rollouts alone → critic can rest. Slow improvement
            # → actor needs better value estimates → critic should update.
            adaptive_acc_growth_window = getattr(self.strategy.args, "adaptive_acc_growth_window", 5)
            acc_history = getattr(self, "adaptive_acc_history", [])
            acc_history.append(adaptive_accuracy)
            if len(acc_history) > adaptive_acc_growth_window:
                acc_history.pop(0)
            self.adaptive_acc_history = acc_history
            if len(acc_history) >= 2:
                adaptive_acc_growth = (acc_history[-1] - acc_history[0]) / max(len(acc_history) - 1, 1)
            else:
                adaptive_acc_growth = -1.0  # insufficient data

            # ── Three-track thresholds (for logging; actual decision is in critic) ──
            acc_growth_th = getattr(self.strategy.args, "adaptive_acc_growth_threshold", 0.005)
            kl_th = getattr(self.strategy.args, "adaptive_kl_threshold", 0.01)
            ent_th = getattr(self.strategy.args, "adaptive_entropy_threshold", 0.2)
            three_track_acc_ok = adaptive_acc_growth < acc_growth_th
            three_track_kl_ok = adaptive_kl > kl_th
            three_track_ent_ok = adaptive_entropy < ent_th
            three_track_trigger = three_track_acc_ok and three_track_kl_ok and three_track_ent_ok

            status["adaptive_kl"] = adaptive_kl
            status["adaptive_accuracy"] = adaptive_accuracy
            status["adaptive_acc_growth"] = adaptive_acc_growth
            status["adaptive_entropy"] = adaptive_entropy
            status["adaptive_entropy_ema"] = entropy_ema
            status["adaptive_entropy_collapse"] = entropy_collapse
            status["three_track_acc_ok"] = three_track_acc_ok
            status["three_track_kl_ok"] = three_track_kl_ok
            status["three_track_ent_ok"] = three_track_ent_ok
            status["three_track_trigger"] = three_track_trigger

        if not self.strategy.is_rank_0():
            return status

        if getattr(self.strategy.args, "adaptive_critic", False):
            kl = _safe_float(status.get("adaptive_kl", 0.0))
            acc = _safe_float(status.get("adaptive_accuracy", 0.0))
            ent = _safe_float(status.get("adaptive_entropy", 0.0))
            growth = _safe_float(status.get("adaptive_acc_growth", -1.0))
            trigger = status.get("three_track_trigger", False)
            acc_ok = status.get("three_track_acc_ok", False)
            kl_ok = status.get("three_track_kl_ok", False)
            ent_ok = status.get("three_track_ent_ok", False)
            print(f"[ACTOR] ep={self.actor_update_count} | "
                  f"kl_ctl={kl_ctl:.4f} | "
                  f"reward={_safe_float(status.get('reward', 0)):.3f} | "
                  f"loss={_safe_float(status.get('policy_loss', 0)):.4f} | "
                  f"3track={'T' if trigger else '.'} "
                  f"(acc={acc_ok} kl={kl_ok} ent={ent_ok}) "
                  f"KL={kl:.3f} growth={growth:+.3f} Ent={ent:.3f}")
        else:
            print(f"[ACTOR] ep={self.actor_update_count} | "
                  f"kl_ctl={kl_ctl:.4f} | "
                  f"reward={_safe_float(status.get('reward', 0)):.3f} | "
                  f"loss={_safe_float(status.get('policy_loss', 0)):.4f}")
            self._last_kl = abs(_safe_float(status.get("kl", status.get("ppo_kl", 0.0))))
        ...
        if step_statuses:
            for i, ss in enumerate(step_statuses):
                ...
                for k, v in ss.items():
                    if k.startswith("_"):
                        continue
                    step_entry[f"actor_{k}"] = _safe_float(v)
                step_entry["step"] = "actor_step"
                step_entry["episode"] = self.actor_update_count
                step_entry["batch"] = i + 1
                _append_metrics(step_entry)

        ep_entry = {f"actor_{k}": v for k, v in status.items()}
        ep_entry["step"] = "actor_episode"
        ep_entry["episode"] = self.actor_update_count
