...
from openrlhf.trainer.ray.adappo_actor import PolicyModelActor
from openrlhf.trainer.ray.adappo_critic import CriticModelActor
#from openrlhf.trainer.ray.adappo_star_critic import CriticModelActorStar as CriticModelActor
...
def train(args):
    ...
    if not ray.is_initialized():
        ...
        def _ray_head_running() -> bool:
            host, port = "127.0.0.1", 6379
            addr = os.environ.get("RAY_ADDRESS", "")
            if addr:
                m = _re.search(r"([\w.\-]+):(\d+)", addr)
                if m:
        ...
        if not _ray_head_running():
            ray_kwargs["object_store_memory"] = int(
                os.environ.get("RAY_OBJECT_STORE_MEMORY", 20_000_000_000)
            )
        ray.init(**ray_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt.save_every_grad_steps", type=int, default=0,
                        help="Save intermediate ckpts by critic gradient step (0 = disabled); "
                             "complements --ckpt.save_steps (iteration-granularity)")
    ...
    parser.add_argument("--adaptive_critic", action="store_true", default=False)
    parser.add_argument("--adaptive_acc_growth_threshold", type=float, default=0.005,
                        help="Three-track: acc_growth below this → Track-1 OK (accuracy stalled)")
    parser.add_argument("--adaptive_kl_threshold", type=float, default=0.01,
                        help="Three-track: |KL| above this → Track-2 OK (policy diverging)")
    parser.add_argument("--adaptive_entropy_threshold", type=float, default=0.2,
                        help="Three-track: entropy below this → Track-3 OK (diversity collapsing)")
    parser.add_argument("--adaptive_acc_growth_window", type=int, default=5,
                        help="Sliding-window size (episodes) for accuracy growth calculation")
    parser.add_argument("--adaptive_kl_ceiling", type=float, default=0.3,
                        help="Hard KL ceiling: |KL| > this → force Critic update unconditionally")
    parser.add_argument("--adaptive_kl_delta", type=float, default=0.1,
                        help="KL relative-surge threshold: |ΔKL|/|KL| > this → force Critic update")
    parser.add_argument("--adaptive_max_skip", type=int, default=4,
                        help="Max consecutive Critic skips before force-update (starvation guard)")
    parser.add_argument("--adaptive_update_rate", type=float, default=None,
                        help="DCPG fixed critic update rate in (0,1]: update the critic on that "
                             "fraction of actor steps; the long-run average matches exactly "
                             "(deterministic uniform schedule via a Bresenham accumulator). "
                             "None disables it and falls back to the three-track gate + max_skip.")
    parser.add_argument("--adaptive_ent_floor", type=float, default=0.05,
                        help="Entropy floor: entropy < this → policy overconfident → force Critic update")
